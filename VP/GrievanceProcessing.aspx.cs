...
public partial class VP_GrievanceProcessing : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    Response.Redirect("VPDashboard.aspx");
                }
    ...
    {
        sql = "SELECT gr.id, gr.GrievanceId, gr.Name, gr.EmailID, gt.CategoryName as grievancetype, gst.SubCategoryName as grievancesubtype, gr.GrievanceDocumentURL, gr.GrievanceDetails, l.UserID, l.Department, dm.RoleID FROM GrievanceRegistration gr left join GrievanceType gt on gt.CategoryID = gr.GrievanceTypeID left join GrievanceSubType gst on gst.SubCategoryID = gr.GrievanceSubTypeID left join Login l on l.UserID = gr.CreatedBy left join DepartmentMaster dm on dm.ID = l.Department where gr.Id = '" + id + "' and GrievanceStatusMasterID = '2' and ForwardedToRoleID = '7' and ResolvedStatus is null and ClosedStatus = '0' ";
        dt = cc.FillTable(sql);
        ...
        try
        {
            ...
            else
            {
                ...
                if (res == "SUSS")
                {
                    ...
                    {
                        sql = "update GrievanceRegistration set GrievanceStatusMasterID = '2', ForwardedToRoleID = '7', ForwardedFromRoleID = '2', ClosedStatus = '0', ClosedByUserID = NULL , ClosedRemarks = NULL, ClosedDate = NULL, resolvedstatus = NULL, resolvedbyuserid = NULL, resolvedremarks = NULL, resolveddocumenturl = NULL, resolveddate = NULL where GrievanceId = '" + lblgrievanceid.Text + "'";
                        res = cc.InsertUpdateDelete(sql);
                        cc.messageweb(this, "Grievance File Upload Failed, Hence Closing Failed");
                        return;
                    }
                    cc.messagewebwithredirect(this, "Closed Successfully", "VPDashboard.aspx");
