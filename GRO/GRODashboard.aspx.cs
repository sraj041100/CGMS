...
public partial class GRO_GRODashboard : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                sql = "select count(*) from grievanceregistration where status = '1'";
                res = cc.ScalerReturnString(sql);
                lbltotalgrievance.Text = res;

                sql = "select count(*) from grievanceregistration where status = '1' and ClosedStatus = '0'";
                res = cc.ScalerReturnString(sql);
                lblopengrievance.Text = res;

                sql = "select count(*) from grievanceregistration where status = '1' and ClosedStatus = '1'";
                res = cc.ScalerReturnString(sql);
    ...
    protected void btnsearch_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                sql = "select gr.id, gr.GrievanceId, l.Name, l.Email, gt.CategoryName as grievancetype, gst.SubCategoryName as grievancesubtype, gr.GrievanceDetails, gr.CreatedOn, l.Department, gsm.StatusName as gstatus from GrievanceRegistration gr inner join Login l on l.UserID = gr.CreatedBy inner join GrievanceType gt on gt.CategoryID = gr.GrievanceTypeID inner join GrievanceSubType gst on gst.SubCategoryID = gr.GrievanceSubTypeID inner join GrievanceStatusMaster gsm on gsm.ID = gr.GrievanceStatusMasterID where GrievanceStatusMasterID = '1' and ForwardedToRoleID = '2' and ResolvedStatus is null and ClosedStatus = '0' and ConfirmedStatus is null and grievanceid = '" + txtgrievanceid.Text + "'";
                dt = cc.FillTable(sql);
                if (dt != null)
                {
                    grdgrievancedetails.DataSource = dt;
    ...
    protected void drpgrievancetype_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                sql = "select gr.id, gr.GrievanceId, l.Name, l.Email, gt.CategoryName as grievancetype, gst.SubCategoryName as grievancesubtype, gr.GrievanceDetails, gr.CreatedOn, l.Department, gsm.StatusName as gstatus from GrievanceRegistration gr inner join Login l on l.UserID = gr.CreatedBy inner join GrievanceType gt on gt.CategoryID = gr.GrievanceTypeID inner join GrievanceSubType gst on gst.SubCategoryID = gr.GrievanceSubTypeID inner join GrievanceStatusMaster gsm on gsm.ID = gr.GrievanceStatusMasterID where GrievanceStatusMasterID = '1' and ForwardedToRoleID = '2' and ResolvedStatus is null and ClosedStatus = '0' and ConfirmedStatus is null and gr.GrievanceTypeID = '" + drpgrievancetype.SelectedValue + "' order by gt.Priority asc, gr.CreatedOn desc";
                dt = cc.FillTable(sql);
                grdgrievancedetails.DataSource = dt;
                grdgrievancedetails.DataBind();
            }
